...
/**
 * Bizsystem – SME Health Check Landing Page
 * -------------------------------------------------------------
 * - Single-file React component (drop-in to Next.js page)
 * - TailwindCSS for styling
 * - Recharts for Radar demo
 * - Lucide icons
 * - Framer Motion for subtle animations
 *
 * Usage (Next.js 13/14):
 *  - If using App Router: put this as `app/page.tsx` (default export)
 *  - If using Pages Router: put this as `pages/index.tsx`
 */
...
export default function LandingPage() {
  const handleCta = (where: string) => {
    // PostHog (optional, safe call)
    // @ts-ignore
    if (typeof window !== "undefined" && window.posthog) {
      // @ts-ignore
      window.posthog.capture("CTA Clicked", { where, page: "landing" });
    }
  };
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-4 gap-6">
            <BenefitCard
              ...
              title="คะแนนสุขภาพองค์กร 650 คะแนน"
              desc="เห็นจุดแข็ง–จุดอ่อนชัดเจน พร้อมคะแนนต่อหมวด"
            />
            <BenefitCard
              icon={<BarChart3 className="h-6 w-6" />}
              title="รายงาน Readiness & Gap"
              desc="คำแนะนำถัดไปแบบ Actionable ใช้คุยกับบอร์ด/ธนาคาร"
            />
            <BenefitCard
              icon={<Users2 className="h-6 w-6" />}
              title="Benchmark กับ SME ไทย"
              desc="เปรียบเทียบระดับความพร้อมกับธุรกิจประเภทใกล้เคียง"
            />
            <BenefitCard
              icon={<BadgeCheck className="h-6 w-6" />}
              title="ฟรี 100%"
              desc="เริ่มใช้ได้ทันที ไม่ต้องใส่บัตรเครดิต"
            />
          </div>
        </div>
      </section>
