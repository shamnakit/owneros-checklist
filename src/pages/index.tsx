...
import { ArrowRight, BadgeCheck, CheckCircle2, FolderClosed, PlayCircle, Rocket, ShieldCheck, Stars, X } from "lucide-react";
...
export default function LandingIndexPage() {
  const title = "Bizzyztem — Mission to the Moon | System Doc Hub & Mission Control for SMEs";
  const description = "ภารกิจสร้างระบบธุรกิจให้ถึงเป้าหมาย • รวมเอกสารระบบไว้ศูนย์กลางเดียว แล้วพาธุรกิจเดินทางได้ต่อเนื่อง • Distance to Moon dashboard พร้อมใช้งาน";
  ...
  const greenlightPct = 82; // ดัชนีความพร้อมรวม (mock)
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900">
        ...
        <header className="sticky top-0 z-30 backdrop-blur bg-white/70 border-b border-slate-200">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-8 text-sm text-slate-600">
              <a href="#value" className="hover:text-slate-900">คุณค่า</a>
              <a href="#product" className="hover:text-slate-900">ตัวอย่าง</a>
        ...
        <section className="relative overflow-hidden">
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 lg:py-24 grid lg:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="space-y-6">
              <h1 className="text-4xl sm:text-5xl font-bold leading-tight tracking-tight">ภารกิจสร้างระบบธุรกิจให้ถึงเป้าหมาย</h1>
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-center gap-4 justify-between">
                  <div>
                    <div className="text-sm text-slate-600">ระยะทางถึงดวงจันทร์</div>
                    <div className="text-2xl font-semibold">{formatNumber(distanceKm)} กม.</div>
                    <div className="text-xs text-slate-500">คำนวณจากคะแนนรวม {totalScore}% (สูตร 384,400 × score/100)</div>
                  </div>
                  ...
                  <div className="text-sm text-slate-700">Greenlight Readiness <span className="font-semibold">{greenlightPct}%</span></div>
                </div>
              </div>
            </motion.div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-xl bg-slate-50 p-4">
                    ...
                    <div className="mt-1 text-lg font-semibold">{band.label}</div>
                    <div className="mt-2 h-1.5 w-full rounded-full bg-slate-200"><div className="h-1.5 rounded-full bg-indigo-600" style={{ width: `${totalScore}%` }} /></div>
                    <div className="mt-1 text-xs text-slate-600">คะแนนรวม {totalScore}% • ระยะทาง {formatNumber(distanceKm)} กม.</div>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        <section id="value" className="py-12 lg:py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid md:grid-cols-3 gap-6">
              <BenefitCard icon={<FolderClosed className="h-6 w-6" />} title="System Doc Hub (ศูนย์รวมเอกสารระบบ)" desc="รวมแผน, SOP/WI, แบบฟอร์ม, นโยบาย, สัญญา, ใบอนุญาต—อยู่ศูนย์กลางเดียว ค้นง่าย แชร์ง่าย" />
              <BenefitCard icon={<CheckCircle2 className="h-6 w-6" />} title="Greenlight Readiness" desc="ตัวชี้วัดความพร้อมของทั้งองค์กร—เอกสารครบ เจ้าภาพครบ ความเสี่ยงอยู่ในเกณฑ์" />
              <BenefitCard icon={<BadgeCheck className="h-6 w-6" />} title="Export-Ready Mission Report" desc="สรุปสถานะภารกิจ + สารบัญเอกสารระบบในคลิกเดียว ใช้คุยกับบอร์ด/ธนาคาร/คู่ค้าได้ทันที" />
            </div>
          </div>
        </section>
        ...
        <section className="py-12 lg:py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <StepCard no={1} title="ตั้งภารกิจ" desc="กำหนดเป้าหมาย/เจ้าภาพ/ETA และแบ่งเฟสบิน" />
              <StepCard no={2} title="อัปโหลด Mission Docs" desc="รวบเอกสารระบบทั้งหมดไว้ศูนย์กลางเดียว" />
        ...
        <section id="pricing" className="py-12 lg:py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <PricingTeaser tier="Crew (Free)" desc="ตั้งภารกิจ 3 รายการ • อัปโหลด Mission Docs • ดูสรุป" button="เริ่มใช้ฟรี" onClick={() => (window.location.href = "/login")} highlight={false} />
              <PricingTeaser tier="Pilot (Pro)" desc="Approver/Reviewer • Alerts • ดาวน์โหลด Mission Report ไม่จำกัด • ทีม 5 ที่นั่ง" button="สนใจ Pro (ฝากคอนแทค)" onClick={() => openInterest("Pro")} highlight={true} />
              <PricingTeaser tier="Commander (Add-on)" desc="Filing/IPO Pack • Data Room (Lite) • สิทธิ์ขั้นสูง • ทีม 15 ที่นั่ง" button="สนใจ Add-on (ฝากคอนแทค)" onClick={() => openInterest("Premium")} highlight={false} />
            </div>
          </div>
        </section>
        ...
        <section id="faq" className="py-12 lg:py-16 bg-slate-50">
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <FAQItem q="ข้อมูลปลอดภัยหรือไม่?" a="จัดเก็บเข้ารหัสและใช้สิทธิ์การเข้าถึงแบบรายบทบาท (Role-based)" />
            <FAQItem q="ใครเห็นอะไรได้บ้าง?" a="Owner/Manager/Reviewer/Approver ตั้งสิทธิ์เข้าถึงได้ตามบทบาท" />
            <FAQItem q="รายงานส่งให้ใครบ้างได้?" a="บอร์ด/ธนาคาร/คู่ค้า ผ่านลิงก์หรือไฟล์ PDF (Mission Report)" />
          </div>
        </section>
  ...
  return (
    <details className="group border-b border-slate-200 py-4">
      <summary className="cursor-pointer list-none flex items-center justify-between text-slate-900 font-medium">
        {q}<span className="transition-transform group-open:rotate-180">⌄</span>
      </summary>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal>
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <button className="rounded-full p-1 hover:bg-slate-100" onClick={onClose} aria-label="Close"><X className="h-5 w-5 text-slate-600" /></button>
        </div>
