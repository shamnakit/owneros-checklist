...
function SourcesPageImpl() {
  ...
  async function api(path: string, init?: RequestInit) {
    const res = await fetch(path, {
      ...(init || {}),
      headers: { ...(init?.headers || {}), "x-org-id": orgId, "Content-Type": "application/json" },
    });
    if (!res.ok) {
    ...
    try {
      ...
      await api("/api/sources", {
        ...
        body: JSON.stringify({
          ...
          name: "Bitrix24 (Deals)",
          kind: "bitrix",
          credentials: { mode: "webhook", baseUrl: bxBaseUrl, userId: bxUserId, webhook: bxWebhook },
        }),
      });
  ...
  async function testSource(id: string) {
    try {
      setTestingId(id);
      const j = await api("/api/sources/test", { method: "POST", body: JSON.stringify({ sourceId: id }) });
  ...
  async function syncSource(id: string) {
    try {
      setSyncingId(id);
      const j = await api("/api/sources/sync", { method: "POST", body: JSON.stringify({ sourceId: id }) });
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="panel-dark">
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {sources.map((s) => (
            <div key={s.id} className="panel-dark p-3 space-y-2">
              ...
              <div className="text-xs muted">
                {s.last_sync ? (
                  <>
                    สถานะล่าสุด: <b>{s.last_sync.status}</b>{s.last_sync.finished_at ? ` • ${new Date(s.last_sync.finished_at).toLocaleString()}` : ""}
          ...
          {!sources.length && (
            <div className="p-4 text-sm muted">ยังไม่มีแหล่งข้อมูล กรอกแบบฟอร์มด้านล่างเพื่อเชื่อมแหล่งข้อมูลแรกของคุณ</div>
          )}
        </div>
      </div>
      ...
      <div className="panel-dark">
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input className="input-dark" placeholder="Base URL (e.g. https://YOUR.bitrix24.com)" value={bxBaseUrl} onChange={(e) => setBxBaseUrl(e.target.value)} />
            <input className="input-dark" placeholder="User ID (e.g. 1)" value={bxUserId} onChange={(e) => setBxUserId(e.target.value)} />
